...
def _fetch_node_names() -> list[str]:
    node_url = os.getenv("ICDC_NODE_MODEL_URL", "")
    if not node_url:
        return []
...
def run_pipeline(
    ...
    if schema_mode == "generated":
        node_url = os.getenv("ICDC_NODE_MODEL_URL", "")
        prop_url = os.getenv("ICDC_PROP_MODEL_URL", "")
        if not node_url or not prop_url:
            raise gr.Error("Set ICDC_NODE_MODEL_URL and ICDC_PROP_MODEL_URL in the environment.")
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        for selected_node in selected_nodes:
            ...
            if not results.empty:
                results_by_node[schema.name] = results.copy()
                ...
                generated_by_node[schema.name] = synth_df.copy()
                ...
                synth_messages.append(f"{schema.name}: generated {len(synth_df)} synthetic rows")
            else:
...
with gr.Blocks(
    ...
    with gr.Row():
        env_text = gr.Textbox(
            ...
            lines=8,
            placeholder="ICDC_NODE_MODEL_URL=...\nICDC_PROP_MODEL_URL=...\nNEO4J_URI=...\nNEO4J_USER=...\nNEO4J_PASSWORD=...",
        )
