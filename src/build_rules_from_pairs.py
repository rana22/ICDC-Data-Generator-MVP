...
def build_columns_section(rule_rows: list[RuleRow], preferred_root: str | None = None) -> list[dict[str, str]]:
    """
    Include every property that participates in any retained rule.
    The root is only a role marker; all retained properties must be present.
    """
    if not rule_rows:
        return []
    ...
    # Put preferred root first if present
    if preferred_root and preferred_root in props:
        columns.append({"name": preferred_root, "role": "root"})
        props.remove(preferred_root)
    else:
        # fall back to a stable root candidate if available
        all_a = {r.A for r in rule_rows}
        all_b = {r.B for r in rule_rows}
        root_candidates = sorted(all_a - all_b)
        if root_candidates:
            root = root_candidates[0]
            columns.append({"name": root, "role": "root"})
            props.remove(root)

    # Everything else that participates in retained rules becomes dependent
    for name in sorted(props):
        columns.append({"name": name, "role": "dependent"})
...
def build_yaml_manifest(
    ...
    columns = build_columns_section(retained_rows, preferred_root=preferred_root)
...
#         # always add validation constrain
#         dependencies.append(
#             {
#                 "type": "constrain",
#                 "A": row.A,
#                 "B": row.B,
#                 **({"rule": "functional_dependency"} if rule_kind == "functional" else {}),
#             }
#         )

#         # add value-selection rule when appropriate
#         if rule_kind in {"strong", "condition"}:
#             dependencies.append(
#                 {
#                     "type": "conditional_distribution",
#                     "A": row.A,
